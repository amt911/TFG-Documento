...
\begin{itemize}
   ...
   %Suelen tener forma de árbol, al hacer preguntas en cadena hasta llegar con la solución.
   ...
   % Es muy similar a otros algoritmos como Dijkstra, pero con la peculiaridad de usar una función heurística, permitiendo así buscar caminos distintos al óptimo.
   ...
   % Si se deseara aplicar a este proyecto sería necesario discretizar el espacio, haciendo perder precisión. Otro problema es que sería necesario aplicarlo periódicamente y en todos los vehículos, cosa que puede resultar muy costosa computacionalmente.

   % Sin embargo, una ventaja es que el estado de los piloto se puede incluir en la funcion heuristica, lo que facilita su integracion en la aplicacion. Tambien se puede aplicar una funcion de suavizado a la trayectoria resultante para mejorar el resultado. Y como el proyecto no se centra en la eficiencia, tambien podria ser un buen candidato como algoritmo de navegacion.
   ...
   % La parte buena es que el estado de los pilotos se podría incluir en la función heurística, facilitando la integración a la aplicación. Y también se puede aplicar una función de suavizado a la trayectoria resultante, pudiendo así mejorar el resultado.
